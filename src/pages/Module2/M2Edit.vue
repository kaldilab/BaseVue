        ...
        <form @submit.prevent="boardEdit" v-else>
          <div class="form-group">
            ...
            >
              ...
              <option
                v-for="item in categoryList('317')"
                :key="item.index"
...
export default {
    ...
    boardEdit() {
      ...
      if (this.title && this.content && this.category) {
        this.$api
          ...
          .then(() => {
            this.$router.push(
              `/module2/view_m2${this.$route.path.match(this.$route.matched[1].regex)[1]}/${this.$route.params.id}`
            ).catch(() => {});
          })
  ...
  computed: {
    ...
    boardTitle: {
      get() {
        return this.$store.state.BoardViewNews.info.title;
      },
      set(value) {
        this.title = value;
      },
    ...
    boardContent: {
      get() {
        return this.$store.state.BoardViewNews.info.content;
      },
      set(value) {
        this.content = value;
      },
    ...
    boardCategory: {
      get() {
        return this.$store.state.BoardViewNews.info.category_srl;
      },
      set(value) {
        this.category = value;
      },
    ...
    boardName() {
      return this.$store.state.BoardViewNews.info.nick_name;
    },
  },
  watch: {
    boardView(newValue, oldValue) {
      if (newValue || oldValue === undefined) {
        this.title = this.$store.state.BoardViewNews.info.title;
        this.content = this.$store.state.BoardViewNews.info.content;
        this.category = this.$store.state.BoardViewNews.info.category_srl;
        this.loading = false;
      }
    },
  },
